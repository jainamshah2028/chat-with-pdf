        ...
        for doc in docs:
            ...
            if matches >= max(1, len(question_words) // 2):  # At least half the keywords
                ...
                for word in question_words:
                    ...
                    highlighted_content = pattern.sub(f'<span class="highlight">{word}</span>', highlighted_content)
...
def load_pdf_with_fallback(file_path):
    ...
    try:
        with st.spinner("📖 Loading with PyPDFLoader..."):
            ...
            st.markdown('<div class="status-success">✅ PDF loaded successfully with PyPDFLoader!</div>', unsafe_allow_html=True)
            return documents
    except Exception as e:
        st.markdown(f'<div class="status-warning">⚠️ PyPDFLoader failed: {str(e)}</div>', unsafe_allow_html=True)
        ...
        with st.spinner("🔄 Trying alternative PDF reader (pdfplumber)..."):
            with pdfplumber.open(file_path) as pdf:
                ...
                if text_content.strip():
                    documents = [Document(page_content=text_content, metadata={"source": file_path, "pages": total_pages})]
                    st.markdown('<div class="status-success">✅ PDF loaded successfully with pdfplumber!</div>', unsafe_allow_html=True)
                    return documents
                else:
                    raise Exception("No text could be extracted from the PDF")
                    
    except Exception as e:
        st.markdown(f'<div class="status-error">❌ All PDF loading methods failed: {str(e)}</div>', unsafe_allow_html=True)
        return None
...
with st.sidebar:
    ...
    provider_options = [
        "Free (Advanced Local Processing)",
        "OpenAI GPT-3.5", 
        "OpenAI GPT-4"
    ]
    ...
    with st.expander("🔧 Advanced Settings", expanded=False):
        ...
        max_results = st.slider("📊 Max Search Results:", 1, 5, 3,
                               help="Maximum number of results to return")
        
        # Search type selection
        search_type = st.selectbox("🔍 Search Algorithm:", 
    ...
    if st.session_state.chat_history:
        ...
        avg_question_length = sum(len(item['question'].split()) for item in st.session_state.chat_history) / len(st.session_state.chat_history)
            ...
            if provider_choice == "Free (Advanced Local Processing)":
                ...
                try:
                    ...
                    if "quota" in str(e).lower() or "429" in str(e):
                        st.markdown('<div class="status-error">❌ OpenAI quota exceeded. Switching to free mode...</div>', unsafe_allow_html=True)
                        answer = advanced_search(st.session_state.processed_docs, question, search_type)
                    else:
