...
def setup_llm_and_embeddings(provider_choice, api_key=None):
    ...
    if provider_choice == "OpenAI GPT-4":
        ...
        # Use pure local text-based search - no embeddings needed
        st.info("✅ Using local text-based search (no external APIs required)")
        return "local_only", None, True
...
with col1:
            ...
            with st.spinner("🔄 Setting up AI provider..."):
                ...
                if not setup_success:
                    ...
                    try:
                        ...
                        st.info(
                            "💡 Try using a different PDF file or check your internet connection if using HuggingFace models."
                        )
    ...
    if question:
        with st.spinner("🤔 Analyzing your question..."):
            ...
            if provider_choice == "Free (Advanced Local Processing)" or llm == "local_only":
                # Use local text-based search (no API calls)
