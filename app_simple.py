...
def search_documents(docs, question, search_type="keyword"):
    ...
    question_words = set(
        w.strip(".,!?;:") for w in question_lower.split() if len(w.strip(".,!?;:")) > 2
    )
...
with col1:
    ...
    if st.session_state.processed_docs:
        ...
        if question:
            ...
            st.markdown(
                f'<div class="user-message"><b>Q:</b> {question}</div>', unsafe_allow_html=True
            )
            st.markdown(
                f'<div class="bot-message"><b>A:</b> {answer}</div>', unsafe_allow_html=True
            )
