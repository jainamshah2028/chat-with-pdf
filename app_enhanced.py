...
def main():
    ...
    # Main header
    st.markdown(
        '<div class="main-header"><h1>🚀 Advanced PDF Chat Assistant</h1></div>',
        unsafe_allow_html=True,
    )
    ...
    if uploaded_files and st.button("🔄 Process Documents", type="primary"):
        with st.spinner("Processing documents..."):
            # Setup AI provider
            llm, embeddings, setup_success = setup_llm_and_embeddings(
                provider_choice, api_key, model_name
            )
            ...
            if all_documents:
                ...
                st.success(
                    f"✅ Successfully processed {len(docs)} text chunks from {len(uploaded_files)} files!"
                )
                ...
                if st.session_state.qa_system and provider_choice != "Free (HuggingFace Only)":
                    try:
                        ...
                        if "quota" in str(e).lower() or "429" in str(e):
                            ...
                            answer = enhanced_qa(
                                st.session_state.processed_docs, question, max_results
                            )
                        else:
